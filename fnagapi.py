...
class Power:
    ...
    def disable_power(self):
        ...
        if logging_enabled: write_at_log("INFO/POWER", "Энергия отключена.")
    ...
    def enable_power(self):
        ...
        if logging_enabled: write_at_log("INFO/POWER", "Энергия включена.")
power = Power()
...
class MusicBox:
    ...
    def set_level(self, level: int):
        if 1 <= level:
            ...
            if logging_enabled: write_at_log("ERROR/MUSICBOX", "Уровень должен быть между 1 и 8.")
    ...
    def disable(self):
        ...
        if logging_enabled: write_at_log("INFO/MUSICBOX", "Музыкальная шкатулка отключена.")
    ...
    def enable(self):
        ...
        if logging_enabled: write_at_log("INFO/MUSICBOX", "Музыкальная шкатулка включена.")
musicbox = MusicBox()
...
class Camera:
    ...
    def open_camera(self):
        ...
        if logging_enabled: write_at_log("INFO/CAMERA", "Камера открыта")

    def close_camera(self):
        write_at_config("EducationEdition", "is_camera_open", 0)
        self.is_camera_open_var = 0
        print("[CAMERA]: Камера закрыта")
        if logging_enabled: write_at_log("INFO/CAMERA", "Камера закрыта")
    ...
    def change_camera(self, cam_id: int):
        ...
        if logging_enabled: write_at_log("INFO/CAMERA", f"Переключение на камеру {cam_id}")
    ...
    def prohibit_camera(self):
        ...
        if logging_enabled: write_at_log("INFO/CAMERA", "Камеры запрещены")

    def allow_camera(self):
        write_at_config("Dev", "-dev_mode", 1)
        write_at_config("Dev", "-disable_cams", 0)
        print("[CAMERA]: Камеры разрешены")
        if logging_enabled: write_at_log("INFO/CAMERA", "Камеры разрешены")
camera = Camera()
...
class Flashlight:
    ...
    def flashlight_power(self, power: bool):
        ...
        if logging_enabled: write_at_log("INFO/FLASHLIGHT", f"Фонарик {state}")
    ...
    def flashlight_disable(self):
        ...
        if logging_enabled: write_at_log("INFO/FLASHLIGHT", "Фонарик отключен.")
    ...
    def flashlight_enable(self):
        ...
        if logging_enabled: write_at_log("INFO/FLASHLIGHT", "Фонанрик включен.")
...
class Entity:
    ...
    def add_entity(self, entity_id: int):
        if entity_id <= 4:
            if entity_id == 4:
                write_at_config("EducationEdition", "biggie_movement_points", 15)
            else:
                write_at_config("EducationEdition", "office_entity", entity_id)
        else:
            print(f"[ENTITY]: Неизвестная сущность: {entity_id}")
            if logging_enabled: write_at_log("ERROR/ENTITY", f"Неизвестная сущность: {entity_id}")
            
        print(f"[ENTITY]: Сущность {entity_id} добавлена")
        if logging_enabled: write_at_log("ERROR/ENTITY", f"Сущность {entity_id} добавлена")
    
    def remove_entity(self, entity_id: int):
        if entity_id <= 3:
            write_at_config("EducationEdition", "office_entity", 0)
        elif entity_id == 4:
            write_at_config("EducationEdition", "biggie_movement_points", 0)
        else:
            print(f"[ENTITY]: Неизвестная сущность: {entity_id}")
            if logging_enabled: write_at_log("ERROR/ENTITY", f"Неизвестная сущность: {entity_id}")
        print(f"[ENTITY]: Сущность {entity_id} удалена")
        if logging_enabled: write_at_log("INFO/ENTITY", f"Сущность {entity_id} удалена")
    ...
    def remove_all_entites(self, player_enabled: bool):
        ...
        if logging_enabled: write_at_log("INFO/ENTITY", f"Все сущности удалены {state}")
    ...
    def ai_set_movement(self, entity_id: int, needed_points: int):
        match entity_id:
            ...
            case _:
                ...
                if logging_enabled: write_at_log("ERROR/AI", f"Неизвестная сущность: {entity_id}")
        print(f"[AI]: Сущность {entity_id} движется на {needed_points} точек")
        if logging_enabled: write_at_log("INFO/AI", f"Сущность {entity_id} движется на {needed_points} точек")
    ...
    def ai_ignore(self, entity_id: int):
        ...
        if logging_enabled: write_at_log("INFO/AI", f"Сущность {entity_id} игнорирует игрока")
    ...
    def ai_unignore(self, entity_id: int):
        ...
        if logging_enabled: write_at_log("INFO/AI", f"Сущность {entity_id} больше не игнорирует игрока")
    ...
    def ai_disable(self):
        ...
        if logging_enabled: write_at_log("INFO/AI", "ИИ отключен")
    ...
    def ai_enable(self):
        ...
        print("[AI]: ИИ включен")
        if logging_enabled: write_at_log("INFO/AI", "ИИ включен")
...
class Time:
    ...
    def set_time(self, time: int):
        if time < 0 and time > 360:
            ...
            if logging_enabled: write_at_log("ERROR/TIME", f"Время не меньше 0 и не больше 360 игровых тактов")
            return 
        write_at_config("EducationEdition", "time", time)
        self.current_time = time
        print(f"[TIME]: Время установлено на: {time} тактов")
        if logging_enabled: write_at_log("INFO/TIME", f"Время установлено на: {time} тактов")
    ...
    def disable_time(self):
        ...
        if logging_enabled: write_at_log("INFO/TIME", "Время отключено.")
    ...
    def enable_time(self):
        ...
        if logging_enabled: write_at_log("INFO/TIME", "Время включено.")
time = Time()
...
class Sesssion:
    ...
    def set_window_mode(self, fullscreen: bool):
        ...
        if logging_enabled: write_at_log("INFO/SESSION", f"Режим окна установлен на {mode}")
    
    def is_running(self):
        return read_at_config("EducationEdition", "is_running")

    def is_program_running(self):
        return read_at_config("EducationEdition", "program_running")
    ...
    def set_level(self, level: int):
        ...
        if logging_enabled: write_at_log("INFO/SESSION", f"Уровень установлен на {level}")

    def get_game_version(self):
        return read_at_config("EducationEdition", "game_version")
    ...
    def start_program(self):
        write_at_config("EducationEdition", "program_running", 1)
    
    def end_program(self):
        write_at_config("EducationEdition", "program_running", 0)
session = Sesssion()